...
const PaymentTeam = () => {
  ...
  const handleScreenShotClick = async (url) => {
      ...
      window.open(tmpURLObj.href, "_blank");
  };
  ...
  return (
      <TableContainer component={Paper}>
                  ...
                  {rows.map((row) => (
                      <TableRow key={row.transactionId} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                          ...
                          <TableCell align="center">
                              <ButtonGroup variant="text" aria-label="Basic button group">
                                  {row.screenshot.map(sh => (
                                      <Button onClick={() => handleScreenShotClick(sh)} key={sh}>One</Button>
                                  ))}
                              </ButtonGroup>
                          </TableCell>
                          ...
                          <TableCell align="right">
                              <ButtonGroup variant="text" aria-label="Basic button group">
                                  <Button variant="contained" onClick={() => handleConfirmOpen(row)}>
                                      Confirm
                                  </Button>
                                  <Button variant="outlined" onClick={() => handleDeinedOpen(row)}>
                                      Deny
                                  </Button>
                              </ButtonGroup>
                          </TableCell>
          ...
          {/* Confirm Dialog */}
          <Dialog open={confirmOpen} onClose={handleConfirmClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
              ...
              <DialogActions>
                  ...
                  <Button onClick={() => handleAgree(selectedRow)} autoFocus>Confirm</Button>
              </DialogActions>
          </Dialog>
          {/* Deny Dialog */}
          <Dialog open={deinedOpen} onClose={handleDeinedClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
              <DialogTitle id="alert-dialog-title">{"Are you Sure?"}</DialogTitle>
              <DialogContent>
                  <DialogContentText id="alert-dialog-description">
                      {selectedRow &&
              ...
              <DialogActions>
                  ...
                  <Button onClick={() => handleDeined(selectedRow)} autoFocus>Deny</Button>
              </DialogActions>
          </Dialog>
      </TableContainer>
  );
